...
let
  ...
  makeVmTestForImage = imageID: image: { testScript, sharedDirs ? {}, diskSize ? null, extraPathsToRegister ? [ ], selinuxEnforcing ? false, machineConfigModule ? generic.defaultMachineConfigModule }: generic.makeVmTest {
    name = "vm-test-fedora_${imageID}";
    inherit system testScript sharedDirs machineConfigModule;
